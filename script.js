const translations = {
  es: {
    ...
    pricing: {
      ...
      perDay: "/día",
      kitePack: "Kite Trip",
      kitePackDetail: "Traslado FOR · desayuno · 8h clases · cenas grupales · downwind guiado · quartos compartidos · fotos/videos · regalo sorpresa",
    ...
    cta: {
      ...
      text: "Solo 9 camas disponibles — reservá con anticipación para asegurar tu lugar. El equipo responde en menos de 2 horas por WhatsApp."
    },
    footer: {
      stay: "ALOJAMIENTO",
      kite: "KITE CENTER",
...
const translationOverrides = {
  es: {
    ...
    kite: {
      ...
      intermediateInc2: "4 horas de clase: R$ 920",
  ...
  pt: {
    ...
    pricing: {
      ...
      kitePack: "Kite Trip",
      kitePackDetail: "Transfer FOR · café da manhã · 8h aulas · jantares em grupo · downwind guiado · quartos compartilhados · fotos/vídeos · presente surpresa",
  ...
  en: {
    ...
    pricing: {
      ...
      kitePack: "Kite Trip",
      kitePackDetail: "FOR transfer · breakfast · 8h lessons · group dinners · guided downwind · shared rooms · photos/videos · surprise gift",
...
const stayNightPrice = "R$ 150";
const kiteTripPrice = "$800 USD";
...
function switchTab(el, type) {
  ...
  if (!submit) return;
  if (type === "kite") {
    submit.dataset.bookingType = "booking.kiteStay";
    submit.dataset.bookingMode = "kite";
  }
  if (type === "clase") {
    submit.dataset.bookingType = "booking.classesOnly";
    submit.dataset.bookingMode = "clase";
  }
  if (type === "home-section") {
    submit.dataset.bookingType = "booking.stay";
    submit.dataset.bookingMode = "stay";
  }
  renderBookingOptions(submit.dataset.bookingMode);
}

function getGuestValue(labelKey) {
  const boxes = Array.from(document.querySelectorAll(".guest-box"));
  const box = boxes.find((item) => item.querySelector(".guest-box-label")?.getAttribute("data-i18n") === labelKey);
  return box?.querySelector(".guest-num")?.textContent.trim() || "";
}
...
function buildAvailabilityMessage() {
  ...
  const dict = translations[lang] || translations.es;
  const submit = document.querySelector(".booking-submit");
  const bookingTypeKey = submit?.dataset.bookingType || "booking.stay";
  const bookingMode = submit?.dataset.bookingMode || "stay";
  const bookingType = getNestedTranslation(dict, bookingTypeKey) || "Alojamiento";
  const dates = document.querySelectorAll(".booking-form input[type=date]");
  const selectedOption = document.querySelector(".room-option input[type=radio]:checked")?.closest(".room-option");
  const optionName = selectedOption?.querySelector(".room-option-name")?.textContent.trim() || "";
  const optionPrice = selectedOption?.dataset.price || selectedOption?.querySelector(".room-option-price")?.textContent.trim().replace(/\s+/g, " ") || "";
  const optionLabel = bookingMode === "stay" ? "Habitación" : "Plan";

  return [
    "Hola, quiero verificar disponibilidad en Wind Life Home.",
    "",
    `Tipo de reserva: ${bookingType}`,
    `Check-in: ${dates[0]?.value || "No informado"}`,
    `Check-out: ${dates[1]?.value || "No informado"}`,
    `Noches: ${getGuestValue("booking.nights") || "No informado"}`,
    `Adultos: ${getGuestValue("booking.adults") || "0"}`,
    `Niños: ${getGuestValue("booking.children") || "0"}`,
    `${optionLabel}: ${optionName || "No informado"}${optionPrice ? ` (${optionPrice})` : ""}`
  ].join("\n");
}
...
function setupInteractions() {
  ...
  document.querySelector(".room-select-row")?.addEventListener("click", event => {
    const opt = event.target.closest(".room-option");
    if (!opt) return;
    document.querySelectorAll(".room-option").forEach(option => option.classList.remove("selected"));
    opt.classList.add("selected");
    const input = opt.querySelector("input[type=radio]");
  ...
  document.querySelector(".booking-submit")?.addEventListener("click", event => {
    event.preventDefault();
    window.open(`${whatsappUrl}?text=${encodeURIComponent(buildAvailabilityMessage())}`, "_blank", "noopener");
  });
...
const galleryPhotos = {
  ...
  sunsets: [
    "assets/img/atardecer/WhatsApp Image 2026-06-25 at 10.20.53 (2).jpeg",
...
const kiteClasses = [
  ...
  },
  {
    ...
    price: "R$ 240",
    period: "1 hora",
    cover: "assets/img/atardecer/WING.png",
    bullets: ["4 horas de clase: R$ 920", "Mismos valores del kitesurf", "Progresión personalizada"]
  },
  {
...
function getBookingMode() {
  return document.querySelector(".booking-submit")?.dataset.bookingMode || "stay";
}

function getBookingOptions(mode = getBookingMode()) {
  if (mode === "kite") {
    return [{
      name: "Kite Trip",
      description: kiteTripDescription,
      price: kiteTripPrice
    }];
  }

  if (mode === "clase") {
    return classBookingPlans;
  }

  return roomData.map((room) => ({
    name: t(room.titleKey),
    description: t("pricing.roomDetail"),
    price: `${stayNightPrice} ${t("booking.perNight")}`
  }));
}

function renderBookingOptions(mode = getBookingMode()) {
  const roomSelect = document.querySelector(".room-select-row");
  if (!roomSelect) return;

  const options = getBookingOptions(mode);
  roomSelect.innerHTML = options.map((option, index) => `
    <label class="room-option ${index === 0 ? "selected" : ""}" data-price="${option.price}">
      <input ${index === 0 ? "checked" : ""} name="booking-option" type="radio">
      <div class="room-option-info">
        <div class="room-option-name">${option.name}</div>
        <div class="room-option-desc">${option.description}</div>
      </div>
      <div class="room-option-price">${option.price}</div>
    </label>
  `).join("");
}
    ...
    if (table) {
      table.innerHTML = roomData.map((room) => `
        <tr>
          ...
          <td class="price">${stayNightPrice}</td>
        </tr>
      `).join("") + `
        <tr style="background:rgba(255,210,0,0.06);">
          ...
          <td class="price" style="color:var(--azul-dark);"><span class="currency">R$</span> 1050</td>
        </tr>
      `;
    }
    ...
    if (table) {
      table.innerHTML = `
        ...
        <tr>
          ...
          <td class="price"><span class="currency">R$</span> 920</td>
        </tr>
        <tr>
        ...
        <tr style="background:rgba(255,210,0,0.06);">
          ...
          <td class="price" style="color:var(--azul-dark);">${kiteTripPrice}</td>
        </tr>
      `;
    }
